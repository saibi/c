...
typedef struct vca_dcs_frame_data {
	...
	// 20150609 hab lens size
	float HVLR[2];
	float HVLBX[2];
	float HVLBY[2];

	// 20150821 step bevel mode (8000x)
	int HVSBEV;

	// 20160708 step bevel parameters (8000x)
	float HVSTBH[2];
	float HVSBP[2];

	// 20160712 inclined cut (8000x)
	int HVIC;
...
#define VCA_DCS_FD_FLAG_HVLR		0x80000100 // huvitz field
#define VCA_DCS_FD_FLAG_HVLBX		0x80000200 // huvitz field
#define VCA_DCS_FD_FLAG_HVLBY		0x80000400 // huvitz field
#define VCA_DCS_FD_FLAG_HVSBEV		0x80000800 // huvitz field
#define VCA_DCS_FD_FLAG_HVSTBH		0x80001000 // huvitz field
#define VCA_DCS_FD_FLAG_HVSBP		0x80002000 // huvitz field
#define VCA_DCS_FD_FLAG_HVIC		0x80004000 // huvitz field
...
// dcs_get_status_val.c
int get_status_value_n_str(char *str, int *p_val, char *status_str);

// dcs_get_chiral.c
int get_chiral_record_value(char *str, char *r_side, char *l_side);

// dcs_parse_ans_ini.c 
int vca_dcs_parse_ans_ini(char *packet, char *status_str);

// dcs_parse_tracer_req.c
...
// dcs_start_angle.c
int find_start_angle_idx(int src_type, float *angle_list, int record_count, float angle);

// dcs_fill_even.c
void fill_even_angle_data(float start_angle, int record_count, int type, float *a_list);

// dcs_data_orientation.c
void relocate_data_orientation(int src_type, float *r_list, float *a_list, int record_count, int start_idx, float std_angle, float *new_r_list, float *new_a_list);
...
#if defined(_saibi_edger_comm_library_) || defined(_SPADEWORK_) || defined(_CAB4000_) || defined(_DICTATOR_EDGER_) || defined(_DICTATOR_DRILL_) || defined(_AUTOCRAT_)
